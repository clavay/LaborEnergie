    ...
    if not DeviceProtocol.objects.using(db_alias).filter(pk=PROTOCOL_ID):
        DeviceProtocol.objects.using(db_alias).bulk_create([
            DeviceProtocol(pk=PROTOCOL_ID,
                           protocol='pysense',
                           description='PySense Interface',
                           app_name='pyscada.pysense',
                           device_class='pyscada.pysense.device',
                           daq_daemon=True,
                           single_thread=True),
        ])
...
def reverse_func(apps, schema_editor):
    ...
    DeviceProtocol = apps.get_model("pyscada", "DeviceProtocol")
    db_alias = schema_editor.connection.alias
    DeviceProtocol.objects.using(db_alias).filter(protocol="pysense").delete()


class Migration(migrations.Migration):
    dependencies = [
        ('laborenergie', '0001_initial'),
